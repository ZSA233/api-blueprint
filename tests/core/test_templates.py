...
def test_template_relative_parts_normalize_cross_platform_separators() -> None:
    assert _template_relative_parts(r"views\_gen_protos") == ("views", "_gen_protos")
...
def test_template_lookup_name_always_uses_forward_slashes() -> None:
    assert _template_lookup_name(r"views\_gen_protos", "protos.go.j2") == "views/_gen_protos/protos.go.j2"
