...
from .naming import to_camel, to_ts_name
...
class TypeScriptRoute:
    ...
    def message_union_helpers(self) -> list["TypeScriptMessageUnionHelper"]:
        helpers: list[TypeScriptMessageUnionHelper] = []
        for contract, alias in (
            (self.protocol.server_message, self.server_message_alias),
            (self.protocol.client_message, self.client_message_alias),
        ):
            helper = self._message_union_helper(contract, alias)
            if helper is not None:
                helpers.append(helper)
        return helpers

    def _message_union_helper(
        self,
        contract: MessageContract | None,
        alias: TypeScriptProto | None,
    ) -> Optional["TypeScriptMessageUnionHelper"]:
        if contract is None or not contract.is_union or contract.name is None or alias is None:
            return None

        variants: list[TypeScriptMessageUnionVariant] = []
        for variant in contract.variants:
            proto = self._ensure_model(variant.model, "MESSAGE", variant.key.upper())
            if proto is None:
                continue
            variants.append(
                TypeScriptMessageUnionVariant(
                    key=variant.key,
                    method=to_camel(to_ts_name(variant.key)),
        ...
        return TypeScriptMessageUnionHelper(
            name=alias.name,
            variants_const=f"{alias.name}Variants",
            handlers_type=f"{alias.name}Handlers",
            dispatch_func=f"dispatch{alias.name}",
            variants=variants,
        )
...
class TypeScriptViewGroup:
    ...
    def message_union_helpers(self) -> list["TypeScriptMessageUnionHelper"]:
        helpers: "OrderedDict[str, TypeScriptMessageUnionHelper]" = OrderedDict()
        for route in self.routes:
            for helper in route.message_union_helpers():
                helpers.setdefault(helper.name, helper)
        return list(helpers.values())


@dataclass(frozen=True)
class TypeScriptMessageUnionVariant:
    key: str
    method: str
    data_type: str
...
@dataclass(frozen=True)
class TypeScriptMessageUnionHelper:
    name: str
    variants_const: str
    handlers_type: str
    dispatch_func: str
