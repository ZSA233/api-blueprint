...
如果希望发布时强制人工审核，仓库维护者还必须在 GitHub 仓库设置中给 `production` environment 配置 `required reviewers`。

没有配置 reviewer 时，workflow 仍然会引用 `production` environment，但不会自动形成真正的人审闸门。

因此 stable 正式发布的远端顺序变为：

1. push stable tag
2. 等待 `Release` workflow 完成构建
3. 在 GitHub 上通过 `production` environment 审核
4. workflow 继续创建 GitHub Release
...
5. 创建 GitHub Release
6. 将远端 `stable` 分支同步到该 stable tag 对应提交
