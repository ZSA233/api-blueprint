...
- `java-server`: emits a preview Spring MVC server scaffold with route service interfaces, stubs, runtime, and Spring controllers. Route DTOs and binary schema helper records live in `<Group>Types.java`. RPC HTTP controllers are usable; STREAM routes use Spring `SseEmitter` bridges and CHANNEL routes generate WebSocket handler/config output, without generating a host session engine, auth, retry, cache, or room management.
- `python-server`: emits Python route service contracts/stubs and a FastAPI HTTP adapter scaffold. `python_package_root` controls the generated package root. The FastAPI adapter covers query/json/form/binary, response envelopes, typed errors, SSE, and WebSocket protocol bridging.
