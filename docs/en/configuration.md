...
- `java-client`: emits a preview Java 17 client using `java.net.http.HttpClient` and Jackson, with transport-neutral route surfaces and a default JDK HTTP adapter. RPC query/json/urlencoded/multipart/binary_schema calls are usable; binary_schema success responses return typed packets, bytes/file raw responses return `ApiRawResponse`, byte streams return `ApiStreamResponse`, route DTOs and binary schema helper records live in `<Group>Types.java`; STREAM and CHANNEL default to explicit unsupported errors so projects can swap in custom transports.
- `java-server`: emits a preview Spring MVC server scaffold with route service interfaces, stubs, runtime, and Spring controllers. Route DTOs and binary schema helper records live in `<Group>Types.java`. RPC HTTP controllers cover query/json/urlencoded/multipart/binary_schema inputs, binary_schema, bytes, file, and byte_stream success responses bypass the JSON envelope, STREAM routes use Spring `SseEmitter` bridges and CHANNEL routes generate WebSocket handler/config output, without generating a host session engine, auth, retry, cache, or room management.
