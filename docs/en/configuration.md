...
- `http-transport`: declares an HTTP server/client combination. `server` can reference a `go-server`, `java-server`, `kotlin-server`, or `python-server`; `clients` can reference Go, TypeScript, Flutter, Kotlin, Java, or Python clients.
