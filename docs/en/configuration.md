...
- `java-server`: emits preview Spring MVC contract-boundary artifacts: `ApiBlueprintOperation` plus route-local composite `@Gen...` annotations under `annotations/`, JavaBean request/response types under `types/`, conversion helpers under `adapters/`, and `spring/GenSpringMvcContractAssertions`. It does not generate production Controllers, Services, Stubs, `GenSpringServerConfig`, SSE/WebSocket bridges, or an HTTP server adapter. Handwritten business Controllers use the generated annotations and types, and tests inject Spring `RequestMappingHandlerMapping` for runtime contract assertions.
...
The Java `java-server` target no longer emits an HTTP server adapter, so it has no generated server resource-limit config. The host Spring application continues to own MVC, filters, interceptors, AOP, request-size limits, and deployment-layer guards. Generated `GenSpringMvcContractAssertions` only checks public routes, operation markers, policy annotations, and visible request/response boundaries.
...
The Java server output layout is `<package>/<root>/runtime/*`, `<package>/<root>/annotations/*`, `<package>/<root>/types/<root>/<group...>/*`, `<package>/<root>/adapters/<root>/<group...>/*`, and `<package>/<root>/spring/*`. `Gen*.java` files are generator-owned; project policy annotations, business Controllers, legacy DTO/VO classes, and services remain host-owned.
...
- `http-transport`: declares an HTTP server/client combination. `server` can reference a `go-server`, `kotlin-server`, or `python-server`; `clients` can reference Go, TypeScript, Flutter, Swift, Kotlin, Java, or Python clients. Java `java-server` is a Spring contract-boundary target, not an HTTP server adapter target.
