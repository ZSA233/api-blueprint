...
- Optional HTTP/Gin adapter with multipart binding and raw bytes/file/stream response writers.
...
For new routes that return bytes, files, or byte streams, prefer `RSP_BYTES(...)`, `RSP_FILE(...)`, or `RSP_BYTE_STREAM(...)` so the raw success response enters ContractGraph and can be recognized by client generators. `HTTP_RAW_RESPONSE()` remains a legacy HTTP adapter escape hatch, but it is not a cross-language contract and does not generate a typed raw-response client surface.
