...
Runtime entrypoints include the aggregate `ApiClient` façade, route clients such as `DemoApi`, DTO `fromJson` / `toJson`, `ApiError`, `lookupApiError`, `ApiTransport`, `ApiRequestOptions`, `ApiStreamBridge`, `ApiChannelBridge`, and binary `Uint8List` codecs. The default HTTP transport uses `package:http` for RPC query/json/urlencoded/multipart/binary_schema requests, applies per-call `ApiRequestOptions.headers` and `ApiRequestOptions.timeout`, decodes binary_schema success responses into typed packets, returns `ApiRawResponse<Uint8List>` for bytes/file raw responses, returns `ApiStreamResponse` for byte streams, uses SSE for STREAM bridges, and uses `web_socket_channel` for CHANNEL bridges. `base_url` / `base_url_expr` are written to HTTP config, not route/runtime clients.
