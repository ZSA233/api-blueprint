...
HTTP body / response kind is also part of shared planning. Request bodies are recorded as `none`, `json`, `urlencoded`, `multipart`, `binary_schema`, or `raw_bytes`, and responses are recorded as `json`, `xml`, `text`, `binary_schema`, `bytes`, `file`, or `byte_stream`. The v1 generators support multipart, raw media responses, and binary schema responses for Go server/client, TypeScript client, and Python server/client; other targets report an explicit unsupported contract error from `api-gen check` when they see those routes.
