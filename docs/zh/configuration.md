...
面向 iOS 宿主应用这类架构时，推荐把生成 Swift SDK 放在 Core Networking 的协议 DTO/client 层，应用自己的 `APIClient` 通过包装或注入 `APITransport` 承载环境选择、鉴权/签名、401、retry、cache、session owner、token storage、日志和监控。生成的 stream/channel route 现在会把 URL 或配置错误作为 `throws` 暴露；连接重试、前后台生命周期和 session 策略仍由宿主实现。生成器只提供协议边界和关键帧，不接管这些运行时策略。
