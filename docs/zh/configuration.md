...
快捷表会推导 `kind`，因此不允许手写 `kind`。`id` 仍必填。快捷表中的 Go `module` 保持 Go module；Python `module` 会映射到 `python_package_root`；Flutter / Kotlin / Java `module` 会映射到 `package`，也可以继续写显式 `package`；Swift 的 `package` 是必填 SwiftPM package identity，Swift `module` 是可选 Swift module stem，省略时默认等于 `package`，不会再 alias 到 `package`；Flutter 和 Java 的 canonical 字段是 `package`，Java 不生成 JPMS `module-info.java`；`[[grpc.python]] module` 同样映射到 `python_package_root`。

核心 target：

- `contract`：省略 `formats` 时默认只输出轻量 `api-blueprint.index.json`，用于 AI agent 和人工维护者离线获得 service / route / target 目录以及推荐 `api-gen inspect` 查询命令；它不内联 schema、typed error refs、route artifact 或 shard 明细。`formats = ["json"]` 才输出完整 `api-blueprint.contract.json`，用于 diff、归档或兜底全量检查；`markdown`、`agent-*` 与 `shards` 适合离线导航包、归档或需要分片快照时开启。
...
- `swift-client`：生成 iOS Swift Package Manager 多 target SDK。`package` 是必填 SwiftPM package identity；`module` 是可选 Swift module stem，省略时默认等于 `package`，生成 `<module>` aggregate、`<module>Runtime` 共享 runtime 和每个 root 一个 `<module><Root>Routes` module。`base_url` / `base_url_expr` 只进入共享 runtime module 的 `HTTPAPIConfig`，route/runtime client 仍依赖 `APITransport`。默认 `runtime_profile = "modern"` 输出 `.iOS(.v15)` 并使用现代 `URLSession` async API；显式 `runtime_profile = "ios14-compat"` 输出 `.iOS(.v14)`，只替换 HTTP transport 为 continuation/delegate 兼容实现，普通 byte stream、SSE 和 WebSocket 均走兼容桥，不改变 wire 协议、DTO、route 名称或 typed error contract。DTO 使用 `Codable, Sendable`；route-local Markdown Binary Schema helper 会生成字段级 packet struct 和 `APIBinaryReader` / `APIBinaryWriter` codec。默认 URLSession transport 支持 query/json/urlencoded/multipart/binary_schema/open、binary_schema/bytes/file/byte_stream 响应、typed error、SSE STREAM bridge 和 WebSocket CHANNEL bridge；`HTTPAPIConfig` 可注入 coding factory、chunk size、error body/SSE/WebSocket payload limit、stream buffer 和 multipart memory threshold，URL/header/multipart/payload 失败通过 `APITransportError` 抛出；生成物不包含 UI、鉴权、签名、401 处理、环境选择、缓存、重试、token storage、session owner 或 singleton app state。
...
- `java-server`：生成 preview Spring MVC server scaffold；输出 route service interface、stub、runtime 与 Spring controller。route DTO 和 binary schema helper record 都位于 `Gen<Group>Types.java`，generated stub 命名为 `Gen<Group>ServiceStub.java`；RPC HTTP controller 覆盖 query/json/urlencoded/multipart/binary_schema 输入，binary_schema、bytes、file、byte_stream 成功响应不套 JSON envelope，byte_stream 通过 Spring streaming response 写出，STREAM 使用 Spring `SseEmitter` bridge，CHANNEL 生成 WebSocket handler/config，但不生成宿主 session engine、鉴权、重试、缓存或 room 管理。
...
Swift client 输出目录是 `Package.swift`、`Sources/<module>/<module>.swift`、`Sources/<module>/Gen<module>.swift`、`Sources/<module>/Transports/HTTP/HTTPAPIClient.swift`、`Sources/<module>Runtime/*`、`Sources/<module>Runtime/Transports/HTTP/*`、`Sources/<module><Root>Routes/<root>/<root>RootClient.swift`、`Sources/<module><Root>Routes/<root>/Gen<root>RootClient.swift` 与 `Sources/<module><Root>Routes/<root>/Routes/<root>/<group...>/*`。`Package.swift` 与 `Gen*.swift` 由生成器拥有并带 `Code generated` header；`Package.swift` 会按当前 roots 重写 SwiftPM aggregate/runtime/root target 列表，`Package(name:)` 使用 `package`，products/targets 使用 `module` 衍生名。package/root façade、route façade、types façade、`APICoding.swift` 和 `HTTPAPIClient.swift` 只在缺失时创建，之后由用户维护且不带 generated header。`runtime_profile` 是运行时代码生成兼容参数，不是协议兼容开关；需要兼容 iOS 14 时显式设为 `ios14-compat`。多 root 项目默认输出为一个 `<module>` aggregate module、一个 `<module>Runtime` 共享 runtime module 和每个 root 一个 `<module><Root>Routes` module，例如 `/runtime` root 输出到 `<module>RuntimeRoutes` 而不是占用共享 runtime module。

面向 `ios-new` 这类宿主架构时，推荐把生成 Swift SDK 放在 Core Networking 的协议 DTO/client 层，应用自己的 `APIClient` 通过包装或注入 `APITransport` 承载环境选择、鉴权/签名、401、retry、cache、session owner、token storage、日志和监控。生成的 stream/channel route 现在会把 URL 或配置错误作为 `throws` 暴露；连接重试、前后台生命周期和 session 策略仍由宿主实现。生成器只提供协议边界和关键帧，不接管这些运行时策略。
...
`api-gen inspect` 直接从配置加载 Blueprint 并构建 ContractGraph，适合按 route、schema、error 或 target 文件索引查询，不需要先生成 `contract.d` 或打开生成代码。`route` / `schema` 子命令可一次传多个查询，`files` / `errors` 可重复 `--route`，便于 agent 在一次命令中拿到一组相关接口的细节。`inspect` 只返回 live ContractGraph 查询结果，不暗示 shard 文件存在，也不会返回默认 shard 路径；需要离线 shard 导航时，应显式生成 `api-blueprint.agent.json` 或 `api-blueprint.contract.d`。`api-gen explain-target` 输出 effective target summary，而不是原始 TOML 片段；它会显示所选 target kind 的关键字段和关键生效值，例如 contract target 省略 `formats` 时仍会显示 `formats = ["index"]`，Wails target 会显示 `version`、`overlay_name`、`frontend_mode`、`include`、`exclude`。`api-gen manifest` 默认输出只含目录的轻量 index；`--profile full` 输出完整 manifest；`--profile agent` 输出 compact agent manifest；`--shards-dir` 输出按 service / route / schema 拆分的 shards。`manifest.version` 是 manifest schema 兼容版本，目前为 `2.0`；`manifest.generator.version` 来自包版本真源。`api-gen check` 会先构建 ContractGraph，再使用共享 planner / capability metadata 做 target dependency、route、request kind 和 response envelope 校验。生成前失败比生成半套代码更容易维护。
