...
- `examples/java/suite/`：手写 Gradle Java 17 application，用于运行 generated Java client 与 Java Spring contract-boundary artifacts 的 compile/smoke。
...
- `example-java-suite`：手动 Java contract-boundary 增强验证；它在临时 workspace 重生成 `java.client` 与 `java.server`，运行 `examples/java/suite`，验证 route 注解、policy meta-annotation、JavaBean request/response 类型、adapter helper 和 generated Java client 最小热路径。它不启动真实 Spring Boot server，也不接入默认 `test`、`example-validation`、`release-preflight` 或 CI。
