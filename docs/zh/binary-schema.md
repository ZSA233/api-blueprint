...
对于 binary schema 响应，Go / Python server adapter 会把 typed packet 返回值编码成 HTTP bytes，Go / TypeScript / Python client 会把成功响应 bytes 解码成 typed packet。尚未实现 binary schema response 的 target 会在 `api-gen check` 阶段明确报 unsupported contract error。
