...
`RouteExecutor` 是 route-level 可复用执行计划。请求级状态必须放入 `Context` 或 metadata store，自定义 provider 应保持 stateless/reentrant。HTTP-only provider 应显式导入 `views/_http` 并通过类似 `httptransport.RequireGin(ctx)` 的 adapter helper 获取 Gin 上下文。

HTTP 行为测试建议放在外部测试包中，例如 `package demo_test`，再导入 route-adjacent `_http` adapter。业务 handler 单测可以继续放在 route 包内，直接构造 typed `ctx/req` 调用 handler。只有确实需要 Gin API 时才使用 `views/_http.RequireGin(ctx)`；这会让相关代码显式绑定 HTTP adapter。
...
`api-blueprint` 可以用于不启动 HTTP 服务的 Wails app。`api-gen-wails` 仍会生成共享 Go / TypeScript 契约层；当 `[golang].transport_adapters = ["wails"]` 时不会生成 Gin HTTP adapter。Wails app shell 只需要注册 generated bindings，不需要导入 `_http` adapter，也不需要启动 Gin/HTTP listener。
...
`transport_adapters = ["wails"]` 表示该 Go core 会被 Wails target 消费，但不生成 Gin HTTP adapter；Wails app 只 import generated bindings 时，构建图不需要包含 Gin。若同一个项目需要 HTTP + Wails，推荐设为 `transport_adapters = ["http", "wails"]` 并配置 `[[wails.targets]]`。`wails` marker 不替代 Wails target；如果写入 `wails`，必须同时配置至少一个 target。完整最小示例见 `examples/wails-hello`。
