...
生成器会覆盖的文件必须命名为 `gen_*` / `Gen*`，或位于 `_gen_*` 目录，并保留 `Code generated ... DO NOT EDIT` header。`impl_*` 与非 `gen_` / 非 `Gen` 文件是用户拥有扩展点，只在缺失时创建，重生成时保留且不带 generated header。Flutter 使用 Dart 风格的 `gen_*.dart`；Swift 使用 `Gen*.swift`；Kotlin 生成器拥有文件命名为 `Gen*.kt`，但 public declaration 名称保持 Kotlin 惯用形态；Java 生成器拥有文件与 public 类型都使用 `Gen*`。
...
`Gen*.swift` 文件由生成器覆盖并带 `// Code generated by api-blueprint (Swift client); DO NOT EDIT.` header。`Package.swift`、package/root façade、route façade、types façade、`APICoding.swift` 和 `HTTPAPIClient.swift` 是 preserved 用户文件，只在缺失时创建，后续重生成不会覆盖用户修改且不带 generated header。

`package` 是 Swift package 和 module 名。`base_url` / `base_url_expr` 只写入 `HTTPAPIConfig`，不会进入 transport-neutral route/runtime client。`runtime_profile` 是运行时代码生成兼容参数，不是协议兼容开关：默认 `modern` 生成 `.iOS(.v15)` 并使用现代 `URLSession` async API；显式 `ios14-compat` 生成 `.iOS(.v14)`，HTTP transport 使用 continuation/delegate 兼容实现。两个 profile 的 DTO、route 名称、wire shape、typed error、binary schema surface 和 public async API 保持一致。

运行时入口包括 package 聚合 façade、root client（例如 `client.api`）、route client（例如 `HelloAPI`）、DTO、`APITransport`、`APIRequestOptions`、`APIRequest<Response>`、`APIError`、`lookupAPIError`、`APIRawResponse`、`APIStreamResponse`、`APIFilePart`、`APIBinaryPayload`、`APIJSONValue`、`APIStreamBridge` 和 `APIChannelBridge`。DTO 使用 `public struct ...: Codable, Sendable`，required 字段非 optional，optional / omitempty 字段为 optional，wire 字段名通过 `CodingKeys` 保留。字符串 / 整数 enum 使用 raw-value `Codable`，multi-variant STREAM/CHANNEL message 使用 associated-value enum 和自定义 codec。Markdown Binary Schema helper 是 route-local `GenBinary.swift`，当前 Swift client 以 `Data` 作为二进制 packet payload 边界。

默认 URLSession transport 实现 RPC query/json/urlencoded/multipart/binary_schema 请求、binary_schema/bytes/file/byte_stream 成功响应、response envelope 解包和 typed error 还原。RPC route 方法接收 `APIRequestOptions(headers:timeout:)` 做单次请求自定义。raw response filename 只从实际 `Content-Disposition` header 解析。STREAM/CHANNEL route surface 通过 `APIStreamBridge` / `APIChannelBridge` 暴露；连接生命周期、backpressure、重连、鉴权刷新和 session 策略仍由宿主应用通过自定义 `APITransport` 或 wrapper 承载。
...
面向 iOS 新架构时，推荐把生成 SDK 放在 Core Networking 的协议 DTO/client 层。应用自己的 `APIClient` 包装或注入 generated `APITransport`，并负责 auth/signing、401、环境切换、retry、cache、session owner、token storage、日志和监控。当前 Swift package target 以单 root 输出为主；多 root 项目应通过 `include` / `exclude` 裁剪一个 root，或拆成多个 Swift target/package。
