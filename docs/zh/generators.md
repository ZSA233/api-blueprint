...
Java target 按 preview 口径使用。`java-client` 使用 Java 17 `java.net.http.HttpClient` + Jackson；`java-server` 使用 Spring MVC + Jackson 生成契约边界壳。生成源码不包含 Gradle/Maven 工程结构，`out_dir` 是 package root，不会追加 `src/main/java`。仓库示例提供 `make example-java-suite`，用于编译和 smoke 新的 Java Spring contract-boundary 生成物。
...
Java server 不生成生产 Controller、Service、Stub、`GenSpringServerConfig`、SSE/WebSocket bridge 或 HTTP server adapter。它输出：
...
route 注解会组合 Spring `@RequestMapping(method = {...})`、`@ApiBlueprintOperation("<operation.id>")`，以及 target-local `spring_policies` 指定的项目注解。Spring/AOP/安全策略不进入 DSL；它们通过 `api-blueprint.toml` 的 Java target provider 配置表达：
...
推荐的强约束接入方式是业务 Controller 继续手写，但方法使用 generated route 注解和 generated request/response 类型；`HttpServletRequest`、登录上下文、AOP 注入对象等非 wire protocol 参数仍由宿主方法自行声明，不进入 DSL request。低风险过渡可以把 route binding 标为 `legacy-bindable`、`legacy-raw` 或把 target 设为 `spring_contract_mode = "annotations-only"`，此时 contract assertion 会保留 route/policy 检查并把 request/response 边界降级为报告，不把旧 POJO 伪装成完整 typed contract。
