...
默认 URLSession transport 实现 RPC query/json/urlencoded/multipart/binary_schema 请求、binary_schema/bytes/file/byte_stream 成功响应、response envelope 解包、typed error 还原、基于 SSE 的 STREAM bridge 和基于 WebSocket 的 CHANNEL bridge。RPC route 方法接收 `APIRequestOptions(headers:timeout:)` 做单次请求自定义。raw response filename 只从实际 `Content-Disposition` header 解析。内置连接 bridge 只处理协议 message/close payload；重连、鉴权刷新、复杂 backpressure 和 session 策略仍由宿主应用通过自定义 `APITransport` 或 wrapper 承载。
