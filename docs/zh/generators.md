...
Java client 生成 transport-neutral route surface、`GenApiTransport`、默认 JDK HTTP adapter 和 `GenApiClient`。用户保留文件包括 `runtime/ApiClient.java`、`routes/<root>/<group...>/<Group>Api.java`、`transports/http/HttpApiClient.java`；其他 `Gen*.java` 文件由生成器覆盖。推荐入口是 `HttpApiClient.create(baseUrl)`，public DTO 使用 `GenDemoTypes.ErrorDemoQuery` / `GenDemoTypes.ErrorDemoResponse` 这类命名。默认 HTTP adapter 实现 RPC query/json/urlencoded/multipart/binary_schema 请求，binary_schema 成功响应解码为 typed packet，bytes/file raw 响应返回 `GenApiRawResponse`，byte stream 返回 `GenApiStreamResponse`，STREAM 和 CHANNEL 默认抛明确 unsupported。
