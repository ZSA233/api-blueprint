...
- AI agent 与人类维护者应优先使用 `api-gen inspect routes/route/files/schema/errors` 按需查询 ContractGraph；`route` / `schema` 可一次传多个查询，`files` / `errors` 可重复 `--route`，避免为多个接口反复启动命令。其次读取轻量 `api-blueprint.index.json` 查看 service / route / target 目录，并按其中的 `queries` 命令继续批量查询相关 route、schema、error 或生成文件。完整 `contract.json` 只在显式 `formats = ["json"]` 或 `api-gen manifest --profile full` 时生成，主要用于 diff、归档和兜底全量检查；`agent.json`、`agent.md` 与 `contract.d/` 主要用于离线导航包和归档。
