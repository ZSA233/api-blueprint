...
HTTP 二进制请求体可用一等 Markdown Binary Schema 描述：route 使用 `.REQ_BINARY("./binary/packet.md")`，schema 文件采用严格 `# packet` / metadata / Markdown table 结构，可直接渲染为文档；Go server 会按 route group 生成 `_gen_binary` parser，并通过共享 `runtime/binary` 复用底层 reader/批量读取 helper，HTTP adapter 支持 `identity` / `gzip` body 解码。
...
`example-compile-check` 适合开发期验证，`example-refresh` 用于接受预期生成变化，`example-validation` 用于严格确认 snapshots 收敛。`example-golang-suite` 是手动端到端增强验证，不进入默认测试、发布预检或 CI。`wails-hello-dev` 会重生成 hello overlay 并启动 Wails v3 GUI；`wails-hello-check` 只严格验证独立 hello 示例。
