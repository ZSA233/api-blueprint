...
- `[[targets]]` 是 canonical 配置入口；也可以使用 `[[go.server]]`、`[[go.client]]`、`[[python.server]]`、`[[transport.http]]` 等快捷表，加载时会 normalize 成 targets。快捷表中的 Python `module` 映射到 `python_package_root`，Kotlin `module` 映射到 `package`。
