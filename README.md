...
- `examples/golang/`、`examples/typescript/`、`examples/kotlin/`、`examples/grpc/protos/`、`examples/grpc/go/`、`examples/grpc/python/` 是生成快照。
...
- gRPC proto 由 `grpc-proto` target 从 ContractGraph 输出，不再把已有 `.proto` 树作为独立真源；`grpc-go` / `grpc-python` 只消费该 proto target 并生成 protobuf/gRPC stub，不生成业务服务实现，可通过 `source_root` 与字段级 proto 元数据迁移旧协议。
