...
`api-blueprint` 用 Python DSL 定义 API 契约，构建统一 `ContractGraph`，再基于同一份协议真源生成文档服务、Go server、TypeScript client、Kotlin Android client、Wails v2/v3 overlay，以及 gRPC proto/service 定义。
...
| 目标 | 状态 | 命令 | 示例 |
|:---|:---:|:---|:---|
| Contract manifest | 可用 | `api-gen` | `api-blueprint.contract.json` |
...
- `examples/golang/`、`examples/typescript/`、`examples/kotlin/`、`examples/grpc/protos/` 是生成快照。
...
- `STREAM` / `CHANNEL` 是长连接契约入口；多消息类型生成单个判别联合消息，legacy `WS().RECV().SEND()` 不进入 vNext 主线。
...
- Contract manifest 会记录 routes、schemas、connections、稳定 hashes、已解析 target plan 和 capability registry，方便 AI agent 理解协议边界。
- `[[targets]]` 是统一配置入口；transport target 使用 `kind = "http-transport"` / `kind = "wails-transport"` 并显式声明 `server` 与 `clients`。
- gRPC proto 由 `grpc-proto` target 从 ContractGraph 输出，不再把已有 `.proto` 树作为独立真源。
