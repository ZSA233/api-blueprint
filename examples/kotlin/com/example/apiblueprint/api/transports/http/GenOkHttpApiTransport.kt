...
public class OkHttpApiTransport(
    ...
    public override fun connectSocket(options: SocketConnectOptions): ApiSocketBridge<*, *> {
        return InMemorySocketBridge<Any?, Any?>()
    }
    ...
    private fun ApiRequest<*>.requestBody(): RequestBody? {
        ...
        if (binary != null) {
            return when (binary) {
                is ByteArray -> binary.toRequestBody("application/octet-stream".toMediaType())
                else -> encodePayload(binary, binarySerializer).toRequestBody("application/json; charset=utf-8".toMediaType())
            }
        }
        return null
    }
...
private class InMemorySocketBridge<Send, Recv> : ApiSocketBridge<Send, Recv> {
    private val messageListeners = mutableListOf<(Recv) -> Unit>()
