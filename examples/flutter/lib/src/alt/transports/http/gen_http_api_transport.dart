...
class HttpApiTransport implements ApiTransport {
  ...
  Future<T> request<T>(ApiRequest<T> request) async {
    final url = _buildUrl(request.path, request.query, request.pathParams);
    final headers = {
  ...
  Uri _buildUrl(String path, Map<String, String?> query, [Map<String, String?> pathParams = const {}]) {
    final normalizedBase = config.baseUrl.replaceFirst(RegExp(r'/$'), '');
    final expandedPath = _expandPath(path, pathParams);
    final normalizedPath = expandedPath.startsWith('/') ? expandedPath : '/$expandedPath';
  ...
  String _expandPath(String path, Map<String, String?> params) {
    ...
    if (RegExp(r'\{[^{}]+\}').hasMatch(expanded)) {
      throw ArgumentError('unresolved path parameter in path: $expanded');
    }
    return expanded;
  }
