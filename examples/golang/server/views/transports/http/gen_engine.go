...
func (conn *HTTPEventStreamConnection) WriteJSON(_ context.Context, payload any) error {
...
func (conn *HTTPEventStreamConnection) CloseJSON(payload any) error {
	if conn == nil || conn.writer == nil {
		return nil
	}
	raw, err := json.Marshal(payload)
...
func (conn *HTTPEventStreamConnection) Abort(code int, reason string) error {
	return conn.CloseJSON(&provider.ConnectionClose{Code: code, Reason: reason})
}

func (conn *HTTPEventStreamConnection) Close(code int, reason string) error {
	return conn.Abort(code, reason)
}
...
func STREAM[Q, B, P, S, CL any](
	...
	engine.GET(relativePath, func(ginCtx *gin.Context) {
		...
		if err := handler(ctx, stream); err != nil {
			_ = stream.Abort(http.StatusInternalServerError, err.Error())
		}
	})
}
...
func (conn *HTTPWebSocketConnection) CloseJSON(payload any) error {
	if conn == nil || conn.conn == nil {
		return nil
	}
	if conn.envelope {
		if err := wsjson.Write(context.Background(), conn.conn, httpWebSocketEnvelope{Type: httpWebSocketEnvelopeClose, Data: payload}); err != nil {
			_ = conn.Abort(1000, "")
			return err
		}
	}
	return conn.Abort(1000, "")
}

func (conn *HTTPWebSocketConnection) Abort(code int, reason string) error {
	if conn == nil || conn.conn == nil {
		return nil
	}
...
func CHANNEL[Q, B, P, S, C, CL any](
	...
	engine.GET(relativePath, func(ginCtx *gin.Context) {
		...
		if err := handler(ctx, channel); err != nil {
			_ = channel.Abort(int(websocket.StatusInternalError), err.Error())
		}
	})
}
...
func makeWSHandler[Q, B, P any](
	...
	return func(ginCtx *gin.Context) {
		...
		if err := executor.RunWSHandler(ctx); err != nil {
			_ = ctx.WsHandle.Conn.Abort(int(websocket.StatusInternalError), err.Error())
		}
	}
}
